...
<Example.Reason>
  The collection path `/groups/{groupId}/tasks` has a `GET` that lists the
...
<Example.Reason>
  ...
  `/groups/{groupId}/tasks/{taskId}`. There's no `GET` on the collection
  path `/groups/{groupId}/tasks`. So without an already-known `taskId`,
