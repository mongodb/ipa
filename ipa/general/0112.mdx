...
For historical reasons, MongoDB APIs used `group` and `groupId` where
the concept today is more accurately described as a project. New APIs must align on the
historical `group` terminology for consistency.
...
`project`, `projects`, or `projectId` when referring to MongoDB Atlas Projects.
...
<Example.Reason>
  `groupId` is the canonical name for the organizational grouping concept in the Atlas Admin API.
</Example.Reason>
...
<Example.Reason>
  `projectId` is a new term. Using it in new APIs diverges from the
  established `groupId` convention in existing APIs.
</Example.Reason>
