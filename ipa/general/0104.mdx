...
<Example.Reason>
  `getGroup` is `camelCase`, matching the casing every generated client
  expects.
</Example.Reason>
...
<Example.Reason>
  `get_group` is snake_case, so generated method names are inconsistent with
  the rest of the API.
</Example.Reason>
...
<Example.Reason>
  `fetchGroup` uses a different verb, so the name no longer signals a Get
