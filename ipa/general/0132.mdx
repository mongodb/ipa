...
resource exposed on its parent.
...
- If the long-running operation is initiated for an existing resource instance,
  the Operation resource must be exposed at the instance level.
...
<Guideline id="IPA-132-operations-endpoint-must-not-define-custom-methods" given="paths" enforcement="rule" effort="check" dependsOn={["IPA-132-operation-must-be-a-read-only-resource"]}>

Operations endpoints **must not** define custom methods unless they have the
backend support for the control actions. Control actions such as cancel result
in side effects and require mutating methods, which the read-only Operations
resource does not otherwise allow.
...
<Example.Reason>
  The Operations endpoints expose only the read methods of the Operations
  resource, with no custom methods attached at the collection or single
  operation level.
</Example.Reason>
...
```yaml
paths:
  /groups/{groupId}/orders/operations/{operationId}:cancel:
    post:
      operationId: cancelOrdersOperation
```

<Example.Reason>
  The cancel control action is defined as a custom method on a single Operation
  without backend support for that action, so the Operations endpoint stops
  being a read-only resource and exposes a control action it cannot honor.
</Example.Reason>
...
<Workflow>
  <Workflow.Step>
    Collect every path containing a custom method suffix (`:customMethod`).
  </Workflow.Step>
  <Workflow.Step>
    Flag any such path whose resource identifier, the part before the colon, is
    an Operations endpoint, i.e. ends with `operations` or `operations/
    {operationId}`.
  </Workflow.Step>
  <Workflow.Step>
    For any flagged path, confirm with the API producer that the backend
    supports the control action; treat the custom method as compliant only when
    that support exists.
  </Workflow.Step>
  <Workflow.Step>
    Confirm Operations endpoints expose no unsupported custom methods at either
    the collection or the single operation level.
  </Workflow.Step>
</Workflow>
...
<Guideline id="IPA-132-operations-endpoint-must-not-be-global" given="paths" enforcement="rule" effort="check" dependsOn={["IPA-132-lros-must-expose-an-operations-endpoint"]}>
...
<Guideline id="IPA-132-operations-endpoint-must-be-a-leaf-resource" given="paths" enforcement="rule" effort="check" dependsOn={["IPA-132-lros-must-expose-an-operations-endpoint"]}>

Operations endpoints **must** be leaf resources. An `operations` segment may
only be followed by a single operation identifier path parameter.
...
<Example.Reason>
  The Operations endpoints end at the `operations` collection and at the single
  operation identified by `{operationId}`, so no resources are nested below the
  Operations resource.
</Example.Reason>
...
<Example.Reason>
  The `logs` resource is nested below a single Operation, so the Operations
  endpoint is no longer a leaf resource.
</Example.Reason>
...
<Workflow>
  <Workflow.Step>
    Collect every path containing an `operations` segment.
  </Workflow.Step>
  <Workflow.Step>
    Confirm each such path ends at the `operations` segment or at a single
    operation identifier path parameter directly following it.
  </Workflow.Step>
  <Workflow.Step>
    Flag any path that nests further segments below the `operations` segment or
    below its operation identifier path parameter.
  </Workflow.Step>
</Workflow>
...
<Guideline id="IPA-132-resource-must-reflect-last-successful-state" given="resource" enforcement="review" effort="explore" implementation dependsOn={["IPA-132-operation-status-must-use-the-standard-status-enum"]}>

Changes to a resource initiated by a long-running operation **must not** be
applied or persisted until the operation reaches a successful terminal state. If
the operation fails, the resource representation **must** reflect its last
...
<Example.Reason>
  While the operation is `IN_PROGRESS` the order still reads as `10`; the
  requested change only becomes visible once the operation reaches `SUCCEEDED`.
</Example.Reason>
...
<Example.Reason>
  The operation reached `FAILED`, so the requested quantity change was never
  persisted and the order still reads as `10`, its last successfully applied
  state.
</Example.Reason>
...
<Example.Reason>
  The order reports the requested quantity of `25` even though the operation
  failed, so the resource exposes a change that was never successfully applied
  and clients cannot tell which state is authoritative.
</Example.Reason>
...
<Workflow>
  <Workflow.Step>
    For each long-running mutation, identify the point at which the
    implementation writes the requested changes to the resource's durable store.
  </Workflow.Step>
  <Workflow.Step>
    Confirm that write happens only once the operation reaches a successful
    terminal state, and that a failed operation leaves the stored resource at
    its last successfully applied state.
  </Workflow.Step>
  <Workflow.Step>
    Flag any long-running mutation that writes requested changes before the work
    succeeds, or that leaves partially applied changes readable after a failure.
  </Workflow.Step>
</Workflow>
