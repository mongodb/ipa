...
Operations endpoints **must not** define custom methods. Control actions such as
cancel result in side effects and require mutating methods, which the read-only
Operations resource does not allow.
...
<Example.Reason>
  ...
  so the Operations endpoint is no longer a read-only resource.
</Example.Reason>
...
<Workflow>
  ...
  <Workflow.Step>
    Confirm Operations endpoints expose no custom methods at either the
    collection or the single operation level.
  </Workflow.Step>
</Workflow>
