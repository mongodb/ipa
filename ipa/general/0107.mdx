...
<Guideline id="IPA-107-must-return-resource-in-response-body" given="update-operation" enforcement="review" effort="reason">

The response body **must** be the same resource returned by the
[Get method](0104.mdx)

- The Update method **must** return the complete resource to avoid complexities
  for clients
...
<Example.Reason>
  The Update response returns `ClusterUpdateResult`, a partial projection rather
  than the full `Cluster` returned by Get. The client cannot observe the
  resource's complete updated state and must issue a follow-up Get to reconcile.
</Example.Reason>
