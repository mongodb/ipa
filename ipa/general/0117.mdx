...
itself a secret. To apply it consistently:

- **Use** `format: password` on any property whose value is itself a secret per
  [IPA-111 — Sensitive Fields](0111.mdx#sensitive-fields).
- **Do not use** `format: password` on a redacted sibling property whose value
