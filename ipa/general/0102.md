...
- Resource identifier components **should** alternate between:
  ...
  - Resource IDs (example: `groupId`, `clusterName`, `orgId`)
    - Resource IDs **must** be identifiers for the resource
    - Resource IDs **should** follow the format `<resourceName>Id`
      ...
      - Example: for collection `clusters`, the resource ID should be `clusterId`
    - Resource IDs **must** be in `camelCase`
    - The resource ID used in the path parameter **must** match the field
      name used in the resource representation
      - This ensures consistency between the URI and the resource schema
      - Example: if the path uses `clusterId`, the resource schema **must** have a
        field named `clusterId`
