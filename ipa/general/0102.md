...
- Resource identifier components **should** alternate between:
  ...
  - Resource IDs (example: `groupId`, `clusterName`, `orgId`)
    ...
    - The resource ID used in the resource identifier (as the URI path parameter) **must** match the field name
      ...
      - Example: if the resource identifier uses `clusterId`, the resource itself **must** have
