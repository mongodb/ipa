...
- Resource identifier components **should** alternate between:
  ...
  - Resource IDs (example: `groupId`, `clusterName`, `orgId`)
    ...
    - The resource ID used in the path parameter **must** match the field name
      used in the resource representation
      - This ensures consistency between the URI and the resource schema
      - Example: if the path uses `clusterId`, the resource schema **must** have
        a field named `clusterId`
