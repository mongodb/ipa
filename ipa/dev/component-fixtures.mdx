...
`<Reason>` is the short prose explanation inside `<Example.Correct>` and
...
### Outside an example (fallback styling)

A `<Reason>` rendered outside an example falls back to neutral colors — the
hairline uses the default border token and the lead inherits the text color.

<Reason>Rendered standalone, no verdict tint is available.</Reason>
